...
To run this deployment all you need is `docker` and the correct `DNS` entries.
...
See this https://infosec.mozilla.org/guidelines/openssh[Mozilla Guide] for `ssh` configuration and create `/etc/ssh/sshd_config.d/99-hardening.conf`. Be carefeul, the sftp link is not correct in the guide for `Ubuntu-24.04`.
...
Generate a bouncer API key and add it to your `.env` file as `CROWDSEC_BOUNCER_KEY`:
...
Start the mailserver and generate DKIM keys:

[source,bash]
----
docker compose up -d mailserver
. .env && docker exec -it ecs_mailserver setup config dkim domain=${HOST}
----

=== DKIM

Add the generated TXT record to your DNS. Get it with:

[source,bash]
----
cat ./data/mailserver/config/rspamd/dkim/rsa-2048-mail-*.public.dns.txt
----
